...
/**
 * Manages reusable FooConvert brand context data.
 */
class Manager {
    ...
    public function register_option(): void {
        register_setting(
            'fooconvert',
            FOOCONVERT_OPTION_BRAND_CONTEXT,
            array(
                'type'              => 'object',
    ...
    public function register_rest_routes(): void {
        $this->register_brand_routes( '/brand-context' );
        $this->register_extract_route( '/brand-context/extract' );
    }

    /**
     * Registers a brand load/save REST route.
     *
     * @param string $route REST route.
     * @return void
     */
    private function register_brand_routes( string $route ): void {
        register_rest_route(
            'fooconvert/v1',
            $route,
            array(
                array(
                    ...
                    'permission_callback' => array( $this, 'can_manage_brand_context' ),
                ),
                array(
                    'methods'             => 'POST',
    ...
    /**
     * Registers a brand extraction REST route.
     *
     * @param string $route REST route.
     * @return void
     */
    private function register_extract_route( string $route ): void {
        register_rest_route(
            'fooconvert/v1',
            $route,
            array(
    ...
    public function handle_save_brand( WP_REST_Request $request ): WP_REST_Response {
        ...
        update_option( FOOCONVERT_OPTION_BRAND_CONTEXT, $brand, false );
    ...
    public function handle_extract_brand( WP_REST_Request $request ) {
        ...
        if ( 'remote' === $mode ) {
            ...
            if ( '' === $url || ! self::is_allowed_remote_url( $url ) ) {
                return new WP_Error( 'fooconvert_brand_invalid_url', __( 'A valid public HTTP or HTTPS URL is required for remote brand extraction.', 'fooconvert' ), array( 'status' => 400 ) );
            }
    ...
    public static function get_saved_brand(): array {
        $brand = get_option( FOOCONVERT_OPTION_BRAND_CONTEXT, null );
    ...
    /**
     * Checks whether the current user can manage brand context.
     *
     * @return bool
     */
    public function can_manage_brand_context(): bool {
        return current_user_can( 'manage_options' );
    }

    /**
     * Checks whether a remote extraction URL is safe to fetch.
     *
