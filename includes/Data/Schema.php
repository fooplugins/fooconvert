...
if ( !class_exists( 'FooPlugins\FooConvert\Data\Schema' ) ) {
    ...
    class Schema extends Base {
        ...
        const FOOCONVERT_CONSENT_LOG_TABLE = 'fooconvert_consent_log';
         ...
         * @return array{events: array<array-key, mixed>, leads: array<array-key, mixed>, consent_log: array<array-key, mixed>}|false Table creation results, or false.
         */
        public function create_event_table_if_needed( ?string $target_version = null ) {
            ...
            $consent_log_creation_results = $this->create_consent_log_table_and_indexes();
            ...
            $results[ $cache_key ] = [
                ...
                'consent_log' => $consent_log_creation_results
            ];
        ...
        /**
         * Create the consent log table that stores proof-of-consent records.
         *
         * Intentionally decoupled from `fooconvert_events` so it can have its
         * own retention (≥ 12 months by default, vs. the 14-day event default),
         * its own deletion surface (destructive admin buttons must not wipe
         * compliance evidence), and its own indexes optimised for DSAR lookups.
         *
         * @return array<array-key, mixed> dbDelta result
         */
        public function create_consent_log_table_and_indexes(): array {
            global $wpdb;

            $charset_collate = $wpdb->get_charset_collate();
            $table_name = parent::get_table_name( FOOCONVERT_DB_TABLE_CONSENT_LOG );
            $timestamp_default = parent::get_timestamp_default();
             ...
             *  - categories is a compact category→state string, e.g. "n=1,p=0,s=1,m=0".
             *  - page_url is the page the consent was given on.
             *  - user_agent_hash is sha256 of the raw UA (site salt applied).
             *  - ip_hash is sha256(truncated_ip + site_salt) — IPv4 truncated to /24,
