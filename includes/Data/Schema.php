...
if ( !class_exists( 'FooPlugins\FooConvert\Data\Schema' ) ) {
    ...
    class Schema extends Base {
        ...
        public function create_event_table_if_needed( ?string $target_version = null ) {
            ...
            /**
             * Fires after the core FooConvert tables have been created or
             * migrated, so that extensions can create their own tables on
             * the same activation / upgrade hook without touching core.
             *
             * Subscribers should be idempotent — `dbDelta()` already is — and
             * should call `Schema::log_table_creation_results()` themselves if
             * they want their creation recorded in the plugin's database log.
             *
             * @since 2.1.0
             *
             * @param string $current_version The target plugin version the
             *                                schema is being migrated to.
             */
            do_action( 'fooconvert_create_tables', $current_version );
