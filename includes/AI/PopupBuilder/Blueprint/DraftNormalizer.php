...
class DraftNormalizer {
    ...
    private static function sanitize_root_attributes( $attributes, string $popup_type ): array {
        $attributes = self::normalize_root_attribute_aliases( $attributes );
        $sanitized  = array();
    ...
    /**
     * Normalizes common flat AI root-attribute aliases into the block attribute tree.
     *
     * @param mixed $attributes Root attributes.
     * @return array<string,mixed>
     */
    private static function normalize_root_attribute_aliases( $attributes ): array {
        $attributes = is_array( $attributes ) ? $attributes : array();

        $background_image = self::get_first_non_empty_alias_value(
            $attributes,
            array( 'background_image', 'backgroundImage', 'background_url', 'backgroundUrl', 'image_url', 'imageUrl' )
        );

        if ( null !== $background_image ) {
            $background_value = is_string( $background_image )
                ? array( 'url' => $background_image )
                : $background_image;

            self::set_nested_default(
                $attributes,
                array( 'content', 'styles', 'background', 'backgroundImage' ),
                $background_value
            );
            self::set_nested_default(
                $attributes,
                array( 'content', 'styles', 'background', 'backgroundSize' ),
                'cover'
            );
            self::set_nested_default(
                $attributes,
                array( 'content', 'styles', 'background', 'backgroundPosition' ),
                '50% 50%'
            );
        }

        $background_color = self::get_first_non_empty_alias_value(
            $attributes,
            array( 'background_color', 'backgroundColor' )
        );
        if ( null !== $background_color ) {
            self::set_nested_default(
                $attributes,
                array( 'content', 'styles', 'color', 'background' ),
                $background_color
            );
        }

        $text_color = self::get_first_non_empty_alias_value(
            $attributes,
            array( 'text_color', 'textColor' )
        );
        if ( null !== $text_color ) {
            self::set_nested_default(
                $attributes,
                array( 'content', 'styles', 'color', 'text' ),
                $text_color
            );
        }
    ...
    /**
     * Sets a nested value when that path is currently empty.
     *
     ...
     * @return void
     */
    private static function set_nested_default( array &$target, array $path, $value ): void {
        if ( empty( $path ) || null === $value || ( is_string( $value ) && '' === trim( $value ) ) || ( is_array( $value ) && empty( $value ) ) ) {
            return;
        }
        ...
        if (
            ...
            || null === $current[ $last ]
            || ( is_string( $current[ $last ] ) && '' === trim( $current[ $last ] ) )
            || ( is_array( $current[ $last ] ) && empty( $current[ $last ] ) )
        ) {
            $current[ $last ] = $value;
        }
    }
    ...
    /**
     * Normalizes one source or a list of sources for alias lookup.
     *
     * @param array<mixed> $source_or_sources Source array or list of source arrays.
     * @return array<int,array<mixed>>
     */
    private static function normalize_alias_sources( array $source_or_sources ): array {
        $is_list = array_keys( $source_or_sources ) === range( 0, count( $source_or_sources ) - 1 );
        ...
        return array( $source_or_sources );
    }

    /**
     * Returns the first non-empty alias value from one or more source arrays.
     *
     * @param array<mixed>      $source_or_sources Source array or list of source arrays.
     * @param array<int,string> $keys Alias keys.
     * @return mixed|null
     */
    private static function get_first_non_empty_alias_value( array $source_or_sources, array $keys ) {
        foreach ( self::normalize_alias_sources( $source_or_sources ) as $source ) {
            foreach ( $keys as $key ) {
    ...
    /**
     * Returns the first scalar alias value as a trimmed string.
     *
     * @param array<mixed>      $source_or_sources Source array or list of source arrays.
     * @param array<int,string> $keys Alias keys.
     * @return string
     */
    private static function get_first_string_alias_value( array $source_or_sources, array $keys ): string {
        $value = self::get_first_non_empty_alias_value( $source_or_sources, $keys );
    ...
    /**
     * Parses simple duration text into minutes.
     *
     * @param string $value Duration text.
     * @return int
     */
    private static function parse_duration_minutes( string $value ): int {
        ...
        $total_minutes = 0.0;
        $match_count   = preg_match_all( '/(\d+(?:\.\d+)?)\s*(days?|d|hours?|hrs?|h|minutes?|mins?|m)\b/i', $value, $matches, PREG_SET_ORDER );
        ...
        foreach ( $matches as $match ) {
            ...
            if ( in_array( $unit, array( 'day', 'days', 'd' ), true ) ) {
                $total_minutes += $amount * 1440;
            } elseif ( in_array( $unit, array( 'hour', 'hours', 'hr', 'hrs', 'h' ), true ) ) {
    ...
    private static function sanitize_content_blocks( $blocks, int $depth = 0, ?array $selected_block_names = null ): array {
        ...
        foreach ( array_values( $blocks ) as $block ) {
            ...
            $block = self::normalize_content_block_aliases( $block );
    ...
    /**
     * Normalizes common AI shorthand content-block shapes into the builder contract.
     *
     * @param array<string,mixed> $block Content block.
     * @return array<string,mixed>
     */
    private static function normalize_content_block_aliases( array $block ): array {
        if ( ! isset( $block['inner_blocks'] ) ) {
            foreach ( array( 'innerBlocks', 'children', 'blocks' ) as $children_key ) {
                if ( is_array( $block[ $children_key ] ?? null ) ) {
                    $block['inner_blocks'] = $block[ $children_key ];
                    break;
                }
            }
        }
        ...
        $name = isset( $block['name'] ) && is_string( $block['name'] ) ? trim( $block['name'] ) : '';
        if ( '' === $name ) {
            $name = self::normalize_content_block_name_alias(
                self::get_first_string_alias_value(
                    array( $block ),
                    array( 'blockName', 'block_name', 'block', 'type', 'kind' )
                )
            );
        ...
        $sources    = array( $attributes, $block );

        switch ( $name ) {
            case 'core/heading':
                if ( '' === self::get_first_string_alias_value( array( $attributes ), array( 'content' ) ) ) {
                    $content = self::get_first_string_alias_value( $sources, array( 'content', 'text', 'title', 'label', 'headline' ) );
                    if ( '' !== $content ) {
            ...
            case 'core/paragraph':
                if ( '' === self::get_first_string_alias_value( array( $attributes ), array( 'content' ) ) ) {
                    $content = self::get_first_string_alias_value( $sources, array( 'content', 'text', 'description', 'copy', 'label' ) );
                    if ( '' !== $content ) {
                        $attributes['content'] = $content;
                    }
                }
            ...
            case 'core/button':
                if ( '' === self::get_first_string_alias_value( array( $attributes ), array( 'text' ) ) ) {
                    $text = self::get_first_string_alias_value( $sources, array( 'text', 'label', 'content', 'title', 'cta', 'cta_text', 'buttonText', 'button_text' ) );
                    if ( '' !== $text ) {
                        $attributes['text'] = $text;
                    }
                }

                if ( '' === self::get_first_string_alias_value( array( $attributes ), array( 'url' ) ) ) {
                    $url = self::get_first_string_alias_value( $sources, array( 'url', 'href', 'link' ) );
                    if ( '' !== $url ) {
                        $attributes['url'] = $url;
                    }
                }
            ...
            case 'core/list':
                if ( ! is_array( $attributes['items'] ?? null ) ) {
                    foreach ( array( 'items', 'values', 'benefits', 'bullets' ) as $items_key ) {
                        if ( is_array( $attributes[ $items_key ] ?? null ) ) {
                            $attributes['items'] = $attributes[ $items_key ];
                            break;
                        }

                        if ( is_array( $block[ $items_key ] ?? null ) ) {
                            $attributes['items'] = $block[ $items_key ];
                            break;
                        }
                    }
                }
            ...
            case 'core/image':
                if ( '' === self::get_first_string_alias_value( array( $attributes ), array( 'url' ) ) ) {
                    $url = self::get_first_string_alias_value( $sources, array( 'url', 'src', 'image', 'image_url', 'imageUrl' ) );
                    if ( '' !== $url ) {
                        $attributes['url'] = $url;
                    }
                }
            ...
            case 'fc/coupon':
            case 'fc/apply-coupon':
                $attributes = self::normalize_coupon_block_attributes( $attributes, $block );
                break;

            case 'fc/countdown':
                $attributes = self::normalize_countdown_block_attributes( $attributes, $block );
                break;
        }
    ...
    /**
     * Maps common non-contract block names to supported block names.
     *
     * @param string $value Raw block alias.
     * @return string
     */
    private static function normalize_content_block_name_alias( string $value ): string {
        $value = trim( $value );
        if ( '' === $value ) {
            return '';
        }
        ...
        $key = strtolower( preg_replace( '/[^a-z0-9]+/', '_', $value ) ?? '' );
        $key = trim( $key, '_' );
        ...
        $aliases = array(
            ...
            'text'           => 'core/paragraph',
            'support'        => 'core/paragraph',
            'supporting_text' => 'core/paragraph',
            'button'         => 'core/button',
            'cta'            => 'core/button',
            'call_to_action' => 'core/button',
            'buttons'        => 'core/buttons',
            'coupon'         => 'fc/coupon',
            'promo_code'     => 'fc/coupon',
            'discount_code'  => 'fc/coupon',
            'apply_coupon'   => 'fc/apply-coupon',
            'countdown'      => 'fc/countdown',
            'timer'          => 'fc/countdown',
            'signup'         => 'fc/sign-up',
            'sign_up'        => 'fc/sign-up',
            'email_capture'  => 'fc/sign-up',
            'lead_form'      => 'fc/sign-up',
            'form'           => 'fc/sign-up',
            'image'          => 'core/image',
            'media'          => 'core/image',
            'list'           => 'core/list',
    ...
    private static function normalize_content_block_attributes( string $name, array $attributes ): array {
        switch ( $name ) {
            ...
            case 'fc/coupon':
            case 'fc/apply-coupon':
                return self::normalize_coupon_block_attributes( $attributes );

            case 'fc/countdown':
                return self::normalize_countdown_block_attributes( $attributes );
    ...
    /**
     * Normalizes coupon shorthand attributes into the nested coupon block shape.
     *
     * @param array<string,mixed> $attributes Block attributes.
     * @param array<string,mixed> $block Source block.
     * @return array<string,mixed>
     */
    private static function normalize_coupon_block_attributes( array $attributes, array $block = array() ): array {
        $sources = array( $attributes, $block );

        if ( is_string( $attributes['code'] ?? null ) ) {
            $code_text          = $attributes['code'];
            $attributes['code'] = array();
        } else {
            $code_attributes = is_array( $attributes['code'] ?? null ) ? $attributes['code'] : array();
            $code_text = self::get_first_string_alias_value(
                array(
                    is_array( $code_attributes['settings'] ?? null ) ? $code_attributes['settings'] : array(),
                    $code_attributes,
                ),
                array( 'text', 'content', 'value', 'code' )
            );
        }

        if ( '' === $code_text ) {
            $code_text = self::get_first_string_alias_value( $sources, array( 'code', 'coupon_code', 'couponCode', 'discount_code', 'discountCode' ) );
        }

        if ( '' !== $code_text ) {
            $attributes['code'] = is_array( $attributes['code'] ?? null ) ? $attributes['code'] : array();
            $attributes['code']['settings'] = is_array( $attributes['code']['settings'] ?? null ) ? $attributes['code']['settings'] : array();

            if ( empty( $attributes['code']['settings']['text'] ) ) {
                $attributes['code']['settings']['text'] = $code_text;
            }

            unset( $attributes['code']['settings']['content'], $attributes['code']['settings']['value'] );
        }

        $label = self::get_first_string_alias_value( $sources, array( 'label', 'title' ) );
        if ( '' === $label ) {
            $label = self::get_first_string_alias_value( $sources, array( 'description' ) );
        }

        if ( '' !== $label ) {
            $attributes['settings'] = is_array( $attributes['settings'] ?? null ) ? $attributes['settings'] : array();
            if ( empty( $attributes['settings']['label'] ) ) {
                $attributes['settings']['label'] = $label;
            }
        }

        $button_text = self::get_first_string_alias_value( $sources, array( 'buttonText', 'button_text', 'copyText', 'copy_text', 'cta', 'cta_text' ) );
        if ( '' !== $button_text ) {
            $attributes['button'] = is_array( $attributes['button'] ?? null ) ? $attributes['button'] : array();
            $attributes['button']['settings'] = is_array( $attributes['button']['settings'] ?? null ) ? $attributes['button']['settings'] : array();

            if ( empty( $attributes['button']['settings']['text'] ) ) {
                $attributes['button']['settings']['text'] = $button_text;
            }

            if ( empty( $attributes['button']['settings']['layout'] ) ) {
                $attributes['button']['settings']['layout'] = 'text-only';
            }
        }
    ...
    /**
     * Normalizes countdown shorthand attributes into the nested countdown block shape.
     *
     * @param array<string,mixed> $attributes Block attributes.
     * @param array<string,mixed> $block Source block.
     * @return array<string,mixed>
     */
    private static function normalize_countdown_block_attributes( array $attributes, array $block = array() ): array {
        $sources  = array( $attributes, $block );
        $settings = is_array( $attributes['settings'] ?? null ) ? $attributes['settings'] : array();

        $specific_value = self::get_first_string_alias_value(
            array( $settings, $attributes, $block ),
            array( 'specificValue', 'specific_value', 'endDate', 'end_date', 'expires_at', 'expiryDate', 'value' )
        );

        if ( '' !== $specific_value ) {
            if ( empty( $settings['specificValue'] ) ) {
                $settings['specificValue'] = $specific_value;
            }
            if ( empty( $settings['value'] ) ) {
                $settings['value'] = $specific_value;
            }
        ...
        $duration = self::get_first_string_alias_value(
            $sources,
            array( 'time_limit', 'timeLimit', 'duration', 'durationText', 'fomo', 'fomoValue', 'minutes' )
        );
        $minutes = self::parse_duration_minutes( $duration );

        if ( $minutes > 0 && empty( $settings['specificValue'] ) ) {
            $settings['mode']      = 'fomo';
            $settings['fomoValue'] = $minutes;
            if ( empty( $settings['expiry'] ) ) {
                $settings['expiry'] = 'session';
            }
        }

        $close_on_expire = self::get_first_non_empty_alias_value(
            array_merge( array( $settings ), $sources ),
            array( 'closeOnExpire', 'close_on_expire' )
        );
        if ( is_bool( $close_on_expire ) && ! isset( $settings['closeOnExpire'] ) ) {
            $settings['closeOnExpire'] = $close_on_expire;
        }
        ...
        unset(
            ...
            $attributes['fomo'],
            $attributes['fomoValue'],
            $attributes['minutes'],
            $attributes['specificValue'],
            $attributes['specific_value'],
            $attributes['endDate'],
            $attributes['end_date'],
            $attributes['expires_at'],
            $attributes['expiryDate'],
            $attributes['value'],
            $attributes['closeOnExpire'],
            $attributes['close_on_expire']
        );
