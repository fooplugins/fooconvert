...
const getContentCardStyle = ( rootAttributes ) => {
	...
	if ( isPlainObject( background?.backgroundImage ) ) {
		backgroundImage = background.backgroundImage?.url || '';
	} else if ( typeof background?.backgroundImage === 'string' ) {
...
const getListItems = ( attributes ) => {
	if ( Array.isArray( attributes?.items ) ) {
		return attributes.items.map( getPlainText ).filter( Boolean );
	}

	const values = String( attributes?.values || '' );
	if ( values.length === 0 ) {
		return [];
	}
	...
	return getPlainText( values )
		.split( /\n+/ )
		.map( ( item ) => item.trim() )
		.filter( Boolean );
};

const renderText = ( tagName, className, text, style = {} ) => {
...
const renderSignupPreview = ( block ) => {
	const inputsSettings =
		getStyleValue( block, 'attributes', 'inputs', 'settings' ) || {};
	const buttonSettings =
		getStyleValue( block, 'attributes', 'button', 'settings' ) || {};
	const emailOnly = Boolean( inputsSettings?.emailOnly );

	return (
		<form
			className="fc-ai-popup-builder__signup-preview"
		...
		>
			{ ! emailOnly && (
				<input
					...
					readOnly
					placeholder={
						inputsSettings?.namePlaceholder ||
						__( 'Your name', 'fooconvert' )
					}
				/>
			) }
			<input
				...
				readOnly
				placeholder={
					inputsSettings?.emailPlaceholder ||
					__( 'Enter your email', 'fooconvert' )
				}
			/>
			<button type="submit">
...
const renderPreviewBlock = ( block ) => {
	...
	const attributes = normalizeDraftBlockAttributes(
		block.name,
		block?.attributes
	);
	const children = Array.isArray( block?.inner_blocks )
		? block.inner_blocks.map( renderPreviewBlock ).filter( Boolean )
		: [];

	switch ( block.name ) {
		case 'core/heading': {
			const level = Number.isFinite( Number( attributes?.level ) )
				? Math.min( 6, Math.max( 1, Number( attributes.level ) ) )
				: 2;
			const style = {
			...
			return renderText(
				`h${ level }`,
				'fc-ai-popup-builder__preview-heading',
				attributes?.content || '',
				style
			);
		}
		...
		case 'core/list':
			return (
				<ul className="fc-ai-popup-builder__preview-list">
					{ getListItems( attributes ).map( ( item, index ) => (
		...
		case 'core/button':
			return (
				<button
					type="button"
					className="fc-ai-popup-builder__preview-button"
				>
					{ attributes?.text ||
						attributes?.content ||
						__( 'Call to action', 'fooconvert' ) }
				</button>
			);
		case 'core/group':
			return (
		...
		case 'core/spacer':
			return (
				<div
					aria-hidden="true"
					style={ { height: attributes?.height || '24px' } }
				/>
			);
		case 'fc/sign-up':
...
export const PopupPreview = ( { draft, templatesBySlug } ) => {
	if ( ! draft ) {
		return (
			<div className="fc-ai-popup-builder__preview-empty">
				<p>
					{ __(
						'Your popup preview will appear here as soon as the AI produces a draft.',
						'fooconvert'
					) }
				</p>
			</div>
	...
	return (
		<div
			className={ `fc-ai-popup-builder__preview fc-ai-popup-builder__preview--${ popupType }` }
		>
			<div
			...
			>
				<div
					...
					style={ {
						...
						backgroundSize: contentStyles.backgroundSize,
						backgroundPosition: contentStyles.backgroundPosition,
						backgroundRepeat: contentStyles.backgroundRepeat,
				...
				>
					<button
						type="button"
						className="fc-ai-popup-builder__preview-close"
						aria-label={ __( 'Close', 'fooconvert' ) }
					>
						×
					</button>
					{ Array.isArray( draft?.content_blocks ) &&
						draft.content_blocks.map( renderPreviewBlock ) }
				</div>
			</div>
		</div>
	);
