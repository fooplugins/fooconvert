...
const ensurePopupBlockCategory = () => {
...
const registerPopupBlock = ( metadata, save ) => {
	...
	registerPopupBlock(
		{
			...
			category: block.name.startsWith( 'woocommerce/' )
				? 'widgets'
				: 'fooconvert',
			attributes: isPlainObject( block?.attribute_schema )
				? block.attribute_schema
				: {},
			parent:
				Array.isArray( block?.parent ) && block.parent.length > 0
					? block.parent
					: undefined,
			ancestor:
				Array.isArray( block?.ancestor ) && block.ancestor.length > 0
					? block.ancestor
					: undefined,
			supports: {
				html: false,
...
export const ensurePopupBuilderBlocksRegistered = ( blockCatalog = [] ) => {
	...
	fooconvertBlockMetadata.forEach( ( metadata ) => {
		...
		registerPopupBlock( metadata, save );
	} );
	...
	switch ( block.name ) {
		case 'core/list': {
			...
			return createBlock(
				'core/list',
				listAttributes,
				extractListItems( attributes ).map( ( item ) =>
					createBlock( 'core/list-item', { content: item }, [] )
				)
			);
		}
		case 'core/button':
...
const buildPopupRootBlock = ( draft, templatesBySlug ) => {
	...
	if ( popupType === 'bar' ) {
		return createBlock( 'fc/bar', rootAttributes, [
			createBlock( 'fc/bar-open-button', {}, [] ),
			createBlock( 'fc/bar-container', {}, [
	...
	if ( popupType === 'flyout' ) {
		return createBlock( 'fc/flyout', rootAttributes, [
			createBlock( 'fc/flyout-open-button', {}, [] ),
			createBlock( 'fc/flyout-container', {}, [
	...
	return createBlock( 'fc/overlay', rootAttributes, [
		createBlock( 'fc/overlay-container', {}, [
...
export const serializeDraftToMarkup = (
	draft,
	templatesBySlug = {},
	blockCatalog = []
) => {
