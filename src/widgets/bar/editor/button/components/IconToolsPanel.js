...
const IconToolsPanel = props => {
    ...
    const iconClose = getIconSetsIcon( iconSets, value?.close?.slug ?? defaults?.close?.slug ?? 'wordpress-reset' );
    const setIconClose = selectedIcon => setIconState( 'close', selectedIcon, { slot: 'button-icon', className: 'button-icon button-icon--close' } );

    const iconOpen = getIconSetsIcon( iconSets, value?.open?.slug ?? defaults?.open?.slug ?? 'wordpress-create' );
    const setIconOpen = selectedIcon => setIconState( 'open', selectedIcon, { slot: 'button-icon', className: 'button-icon button-icon--open' } );
            ...
            { currentAction === 'toggle' && (
                ...
                >
                    <IconPickerControl
                        ...
                        iconSets={ iconSets }
                        help={ __( 'The icon displayed when clicking the button will open the bar.', 'fooconvert' ) }
                    />
                </ToolsPanelItem>
            ) }
        </ToolsPanel>
