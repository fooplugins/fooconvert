...
const matchesMediaItem = ( block, mediaItem ) => {
    if ( block?.name !== "core/image" || !isPlainObject( block?.attributes ) || !isPlainObject( mediaItem ) ) {
        return false;
    }
    ...
    if ( Number.isFinite( blockId ) && blockId > 0 && Number.isFinite( mediaId ) && mediaId > 0 ) {
        return blockId === mediaId;
    }

    return typeof block.attributes.url === "string" && typeof mediaItem.url === "string" && block.attributes.url === mediaItem.url;
};

const buildImageBlock = ( mediaItem ) => ( {
    name: "core/image",
    attributes: {
        id: Number.isFinite( Number( mediaItem?.id ) ) ? Number( mediaItem.id ) : undefined,
...
const replaceFirstImageBlock = ( blocks, imageBlock ) => {
    for ( let index = 0; index < blocks.length; index += 1 ) {
        ...
        if ( Array.isArray( block.inner_blocks ) && replaceFirstImageBlock( block.inner_blocks, imageBlock ) ) {
            return true;
        }
    }
...
const getImageInsertIndex = ( blocks ) => {
    const actionIndex = blocks.findIndex( block => [ "fc/sign-up", "core/buttons", "core/button" ].includes( block?.name ) );
...
const removeMatchingImageBlocks = ( blocks, mediaItem ) => {
    return blocks.reduce( ( nextBlocks, block ) => {
        ...
        if ( Array.isArray( block.inner_blocks ) ) {
            nextBlock.inner_blocks = removeMatchingImageBlocks( block.inner_blocks, mediaItem );
        }
...
export const applyMediaItemToDraft = ( draft, mediaItem ) => {
    if ( !isPlainObject( draft ) || !isPlainObject( mediaItem ) || typeof mediaItem?.url !== "string" || mediaItem.url.length === 0 ) {
        return draft;
    }

    const nextDraft = cloneDeep( draft );
    nextDraft.content_blocks = Array.isArray( nextDraft.content_blocks ) ? nextDraft.content_blocks : [];
    ...
    nextDraft.content_blocks.splice( getImageInsertIndex( nextDraft.content_blocks ), 0, imageBlock );
    return nextDraft;
};
