...
const normalizeMediaItem = value => {
    ...
    const previewUrl = `${ value?.previewUrl || value?.preview_url || url }`.trim();
...
export const normalizeAiBuilderMetadata = value => {
    ...
    const response = isPlainObject( metadata?.response ) ? metadata.response : {};
    const validation = isPlainObject( response?.validation ) ? response.validation : null;
    const popupDraft = isPlainObject( response?.popup_draft ) ? response.popup_draft : null;
    ...
    return {
        ...
        response: {
            ...
            clarifying_question: `${ response?.clarifying_question || "" }`.trim(),
            ...
            media_items: Array.isArray( response?.media_items )
                ? response.media_items.map( normalizeMediaItem ).filter( Boolean )
                : defaults.response.media_items,
        },
        options: {
            generate_images: Boolean( metadata?.options?.generate_images ),
            force_image_generation: Boolean( metadata?.options?.force_image_generation ),
        },
    };
};
...
export const hasAiBuilderMetadata = value => {
    ...
    return metadata.messages.length > 0
