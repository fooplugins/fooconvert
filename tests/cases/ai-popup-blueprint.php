...
namespace {
    ...
    $external_shorthand_draft = PopupBlueprint::sanitize_popup_draft(
        array(
            'popup_type'            => FOOCONVERT_POPUP_TYPE_OVERLAY,
            'root_attributes'       => array(
                'background_image' => 'https://example.test/first-time-shopper-background.webp',
            ...
            'content_blocks'        => array(
                array(
                    ...
                    'attributes' => array(
                        ...
                        'description' => 'Limited time offer - expires in 2 hours!',
                    ),
                ),
                array(
                    'type'       => 'countdown',
            ...
            'conversion_rationale' => 'Urgency plus exclusivity plus a clear value proposition.',
            'notes'                => 'Recovered from an external shorthand response.',
        )
    );

    Assertions::same(
        FOOCONVERT_POPUP_TYPE_POPUP,
        $external_shorthand_draft['popup_type'],
        'External overlay popup types should normalize to the builder-facing popup type.'
    );

    Assertions::same(
        array( 'fc/coupon', 'fc/countdown', 'core/button' ),
        array_column( $external_shorthand_draft['content_blocks'], 'name' ),
        'External shorthand content block types should map to supported builder block names.'
    );

    Assertions::same(
        'WELCOME15',
        $external_shorthand_draft['content_blocks'][0]['attributes']['code']['settings']['text'] ?? '',
        'Coupon shorthand code values should map to the nested coupon code text setting.'
    );

    Assertions::same(
        120,
        $external_shorthand_draft['content_blocks'][1]['attributes']['settings']['fomoValue'] ?? 0,
        'Countdown shorthand duration values should map to the countdown FOMO value in minutes.'
    );

    Assertions::same(
        'Shop Now',
        $external_shorthand_draft['content_blocks'][2]['attributes']['text'] ?? '',
        'CTA shorthand blocks should preserve button text.'
    );

    Assertions::same(
        'https://example.test/first-time-shopper-background.webp',
        $external_shorthand_draft['root_attributes']['content']['styles']['background']['backgroundImage']['url'] ?? '',
        'Flat background image aliases should map to the nested popup content background image.'
    );

    Assertions::same(
        array( 'Urgency plus exclusivity plus a clear value proposition.' ),
        $external_shorthand_draft['conversion_rationale'],
        'String conversion rationale values should be retained as one-item lists.'
    );
